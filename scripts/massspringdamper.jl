...
x₁ = cumsum(ones(Np)) ./ Np
y₁ = vcat(x₁, zeros(Np))
ywant = hcat(y₁, zeros(Ns), zeros(Ns))
...
L = inv(H'H + I(Ns * o) * 1e-2) * H'    # Levenberg-Marquaad (or something along that way) with ϵ = 1e-2

# ILC loop
u0 = zeros(Ns)  # Initialize position, velocity and control
tspan = (0.0, T[end])
for i in 1:Niter
    ...
    yⁱ[:, :, i] = vcat(xⁱ[:, :, i], vⁱ[:, :, i])
    if i < Niter
        αrray[:, :, i+1] = αrray[:, :, i] + reshape(L * (ywant[:] - yⁱ[:, :, i][:]), Ns, o)
    end
end
...
for i in 1:N
    ...
    scatter!(ax2, T, ywant[N+i, :], color=:black, marker=:x, markersize=8, label=L"$\overline{y}$")  # Desired 
end
...
lines!(ax4, t_vals, vcat(u_vals...), color=:black)
